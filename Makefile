...
VFILES:=Lectures/Coq_Lecture1_Basics.v\
  Lectures/Coq_Lecture2_Induction_List.v\
  Lectures/Coq_Lecture3_Tactics_Logic.v\
  Lectures/Coq_Lecture4_Program.v\
  Lectures/Coq_Lecture5_Hoare.v\
  Assignments/Assignment2.v\
  Assignments/Assignment3.v\
  Assignments/Assignment4.v\
  Assignments/Assignment5.v
...
VO=vo
VOFILES:=$(VFILES:.v=.$(VO))
VOFILES0=$(patsubst Lectures/%,%,$(filter Lectures/%,$(VOFILES)))
VOFILES1=$(patsubst Assignments/%,%,$(filter Assignments/%,$(VOFILES)))
...
NATIVEFILES0=$(patsubst Lectures/%,%,$(filter Lectures/%,$(NATIVEFILES)))
NATIVEFILES1=$(patsubst Assignments/%,%,$(filter Assignments/%,$(NATIVEFILES)))
...
install:
	cd "Assignments" && for i in $(NATIVEFILES1) $(GLOBFILES1) $(VFILES1) $(VOFILES1); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/Assignments/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Assignments/$$i; \
	done
	cd "Lectures" && for i in $(NATIVEFILES0) $(GLOBFILES0) $(VFILES0) $(VOFILES0); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/Lectures/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/Lectures/$$i; \
	done
...
uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Assignments && rm -f $(NATIVEFILES1) $(GLOBFILES1) $(VFILES1) $(VOFILES1) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Assignments" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/Lectures && rm -f $(NATIVEFILES0) $(GLOBFILES0) $(VFILES0) $(VOFILES0) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "Lectures" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
