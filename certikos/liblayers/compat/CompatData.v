...
Class CompatRel D1 D2 `{ops: !CompatRelOps D1 D2} :=
  {
    match_compose:
      forall d1 m2 f m2' j,
        ...
        Mem.inject j m2 m2' ->
        inject_incr (Mem.flat_inj glob_threshold) j ->
        match_AbData d1 m2' (compose_meminj f j);

    store_match_correct:
      forall abd m0 m0' f b2 v v' chunk,
        match_AbData abd m0 f ->
        (forall i b,
           In i new_glbl ->
           find_symbol i = Some b -> b <> b2) ->
        Mem.store chunk m0 b2 v v' = Some m0' ->
        match_AbData abd m0' f;

    alloc_match_correct:
      forall abd m'0  m'1 f f' ofs sz b0 b'1,
        match_AbData abd m'0 f->
        ...
        (forall i b,
           In i new_glbl ->
           find_symbol i = Some b -> b <> b0) ->
        match_AbData abd m'1 f';

    free_match_correct:
      forall abd m0 m0' f ofs sz b2,
        match_AbData abd m0 f->
        (forall i b,
           In i new_glbl ->
           find_symbol i = Some b -> b <> b2) ->
...
Definition crel data1 data2
  ...
  `{!CompatRelOps (cdata data1) (cdata data2)}
  `{!CompatRel (cdata data1) (cdata data2)}:
  path compatrel (cdata data1) (cdata data2) :=
  path_inj (one_crel data1 data2).
