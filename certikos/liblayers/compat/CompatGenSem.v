...
Section SEMANTICS.
  ...
  Proof.
    split.
    * intros _ _ _ _ [l m d v d' H].
      eapply semprops_well_typed.
      eassumption.
    * intros _ _ _ _ b [l m d v d' H] Hb.
      exact Hb.
    * intros _ _ _ _ b ofs p [l m d v d' H] Hvalid Hperm.
      exact Hperm.
    * intros _ _ _ _ [l m d v d' H].
      simpl.
      eauto with mem.
    * intros _ _ _ _ [m2 d2] vargs2 [vargs1 m1 d1 v d' H] Hm2 Hvargs2.
    ...
    * intros _ _ _ _ ι [m2 d2] vargs2 Hι [vargs1 m1 d1 v d' H].
      intros Hm Hvargs.
      lift_auto. subst. (* d1 = d2 *)
      exists ι, v, (m2, d').
      repeat split.
      ...
      + rewrite H0 in H1; discriminate.
    * intros _ _ _ _ v2 [m2 d2] [l m1 d v1 d1 Ha] H.
      simpl in H.
  ...
  Global Instance extcall_generic_invariants f:
    PreservesInvariants f ->
    ExtcallInvariants (D := cdata D) (sextcall_generic_info f).
  Proof.
    intros Hf.
    split.
    * intros vargs m d m' d' vres H Hd.
      inversion H; subst.
    ...
    * intros vargs m d m' d' vres H Hd.
      ...
      eassumption.
    * intros vargs m d m' d' vres H.
      inversion H; subst.
      reflexivity.
    * intros l m d v m' d' H.
      inversion H; subst.
      intros; split.
      + eapply semprops_inject_neutral.
        eassumption.
  ...
  Inductive primcall_generic_sem (f: T):
    Asm.regset -> mwd (cdata D) -> Asm.regset -> mwd (cdata D) -> Prop :=
      primcall_generic_sem_intro rs m d d':
        semof f nil d Vundef d' ->
        primcall_generic_sem f rs (m, d) (Asm.nextinstr rs) (m, d').

  Lemma primcall_generic_determ (f: T) rs m rs1 rs2 m1 m2:
    primcall_generic_sem f rs m rs1 m1 ->
    primcall_generic_sem f rs m rs2 m2 ->
    m1 = m2 /\ rs1 = rs2.
  Proof.
    intros H1 H2.
    inv H1.
...
Ltac inv_generic_sem H :=
  lazymatch type of H with
    ...
    | sextcall_generic_sem ?f _ _ _ _ _ =>
    ...
    | primcall_generic_sem ?f _ _ _ _ =>
