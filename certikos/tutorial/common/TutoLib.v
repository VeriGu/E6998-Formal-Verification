...
(** Tactics like the ones in VST to temporarily hide some definition to save
  space on the screen. *)
...
Tactic Notation "abbreviate" constr(y) "as" ident(x) :=
  (first [is_var y |
          let x' := fresh x in
          pose (x' := @abbreviate _ y);
          change y with x']).

Tactic Notation "abbreviate" constr(y) "as" ident(x) "in" hyp(H) :=
  (first [is_var y |
          let x' := fresh x in
          pose (x' := @abbreviate _ y);
          change y with x' in H]).

Ltac unabbreviate ab :=
  repeat match goal with
  | H : context[ab] |- _ => unfold ab, abbreviate in H
  | |- context[ab] => unfold ab, abbreviate
  end; clear ab.

Ltac unabbreviate_all :=
  repeat match goal with
  | A := @abbreviate _ _ |- _ => unabbreviate A
  end.
  ...
  | [H: MakeProgramSpec.make_globalenv _ _ = _ |- _] =>
      ...
      | [H1: exists _,
          ...
          Genv.find_funct_ptr _ _ = Some _ |- _] =>
            ...
            match type of Hfp with
            | _ = ?FP => abbreviate FP as FUNDEF in Hfp
            end
      | _ => idtac
      end
  end.
